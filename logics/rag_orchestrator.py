...
async def a_get_query_response(request: RagOrchestratorRequest,
    ...
    request.query = f"{request.query}. {request.text_by_card}." if request.text_by_card != None and len(
        request.text_by_card) > 0 else request.query
...
async def a_do_query(request: RagOrchestratorRequest,
    ...
    return RagOrchestratorResponse(rag_query_result.response,
                                   ...
                                   None,
                                   None if len(
                                       rag_query_result.finish_reason) == 0 else rag_query_result,
                                   clog=clog or None)
...
async def check_msd_question(request: RagOrchestratorRequest,
    ...
    clog_last_status = CLog(ret_code=200, err_desc=None,
                            id_event=clog_settings.msd_elencodomande, params=clog_params)
    ...
    try:
        ...
        if len(list_forms.listaDomande) > 1:
            if string.is_null_or_empty_or_whitespace(request.text_by_card) or len(intent_result.numero_domus) == 0:
                ...
                user_form_application = next((domanda for domanda in list_forms.listaDomande if domanda.numeroDomus == str(
                    intent_result.numero_domus[0])), None)
        else:
        ...
        clog_last_status = CLog(ret_code=200, err_desc=None,
                                id_event=clog_settings.msd_dettagliodomande, params=clog_params)
        ...
        return await a_do_query(request, completion_prompt_data, language_service, enriched_query, logger, session,
                                domusData=str(
                                    form_application_details.model_dump()),
        ...
        return RagOrchestratorResponse("", "", None, "",
                                       MonitorFormApplication(
                                           event_type=EventMonitorFormApplication.application_error),
                                       clog_last_status)
