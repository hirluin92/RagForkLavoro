...
async def a_get_query_response(
    request: RagOrchestratorRequest, logger: Logger, session: ClientSession, consumer: LLMConsumer
    ...
    request.query = (
        request.text_by_card if request.text_by_card != None and len(request.text_by_card) > 0 else request.query
    )
    ...
    logger.track_event(
        event_types.rag_orchestrator_tag_info_by_db,
        {"tag_info": json.dumps(asdict(tag_info), ensure_ascii=False).encode("utf-8")},
    )

    tag_info.id_monitoring_question = (
        request.configuration.id_monitor_form_app_integration
        if request.configuration and request.configuration.id_monitor_form_app_integration
        else tag_info.id_monitoring_question
    )
    ...
    tag_info.enable_cqa = (
        ...
        and request.configuration.enable_cqa
        or ((request.configuration == None or request.configuration.enable_cqa == None) and tag_info.enable_cqa)
        else False
    )
    tag_info.enable_enrichment = (
        True
        if request.configuration
        and request.configuration.enable_enrichment
        or (
            (request.configuration == None or request.configuration.enable_enrichment == None)
            and tag_info.enable_enrichment
        )
        else False
    )

    logger.track_event(
        event_types.rag_orchestrator_tag_info_by_request,
        {"tag_info": json.dumps(asdict(tag_info), ensure_ascii=False).encode("utf-8")},
    )
    ...
    if tag_info.enable_cqa:
        ...
        if cqa_result:
            return RagOrchestratorResponse(cqa_result.text_answer, None, cqa_result.cqa_data, None)
    ...
    ) = await a_get_prompts_data(request.prompts, list_prompt_version_info, logger, session)
    ...
    if (
        request.llm_model_id != enrichment_prompt_data.llm_model
        or request.llm_model_id != completion_prompt_data.llm_model
    ):
    ...
    if tag_info.enable_enrichment:
        # Compute enrichment
        enriched_query = await language_service.a_do_query_enrichment(
        ...
        if enriched_query.end_conversation:
            ...
            return RagOrchestratorResponse(answer_to_return, enriched_query.standalone_question, None, None)
    ...
    if tag_info.enable_cqa and tag_info.enable_enrichment:
        ...
        if enriched_query.standalone_question != request.query:
            ...
            if cqa_result:
                logger.track_event(
                    event_types.cqa_with_enrichment_event,
                    {"originalQuestion": request.query, "normalizedQuestion": enriched_query.standalone_question},
                )
                return RagOrchestratorResponse(
                    cqa_result.text_answer, enriched_query.standalone_question, cqa_result.cqa_data, None
                )
    ...
    if tag_info.id_monitoring_question == EnumMonitorFormApplication.OnlyRag.value:
        return await a_do_query(
            request, completion_prompt_data, language_service, enriched_query, logger, session, consumer
        )
    ...
    if tag_info.id_monitoring_question == EnumMonitorFormApplication.Rag_MonitoringQuestion.value and (
        result is None or (result.monitor_form_application is None and result.clog is not None)
    ):
        return await a_do_query(
            request,
            completion_prompt_data,
...
async def a_do_query(
    ...
    # Compute completion
    rag_query_result = await language_service.a_do_query(
        request, completion_prompt_data, logger, session, consumer, domusData
    )

    return RagOrchestratorResponse(
        rag_query_result.response,
...
async def check_msd_question(
    ...
    if not redisCache:
        ...
        if string.is_null_or_empty_or_whitespace(request.user_fiscal_code) or string.is_null_or_empty_or_whitespace(
            request.token
        ):
            # User not authenticated
            return RagOrchestratorResponse(
                "", "", None, "", MonitorFormApplication(event_type=EventMonitorFormApplication.user_not_authenticated)
            )

        prompt_settings = PromptSettings()
        (domus_form_application_code, domus_form_application_name) = await a_get_form_application_name_by_tag(
            prompt_settings.config_container, tag, logger
        )
        ...
        try:
            try:
                list_forms = await domus.a_get_form_applications_by_fiscal_code(
                    DomusFormApplicationsByFiscalCodeRequest(
                        request.user_fiscal_code,
                        request.token,
                        domus_form_application_code,
                        (
                            intent_result.stato_domanda[0]
                            if intent_result.stato_domanda and len(intent_result.stato_domanda) > 0
                            else None
                        ),
                    ),
                    session,
                ...
                logger.track_event(
                    event_types.event_track_log_intent_result,
                    {"Intent_result": json.dumps(intent_result, default=custom_serializer)},
                )
                logger.track_event(
                    event_types.event_track_log_intent_result,
                    {"list_forms": json.dumps(list_forms, default=custom_serializer)},
                )
            ...
            if list_forms and (
                list_forms.errore or not string.is_null_or_empty_or_whitespace(list_forms.messaggioErrore)
            ):
            ...
            if list_forms is None or not list_forms.listaDomande:
                ...
                return RagOrchestratorResponse(
                    ...
                    None,
                    "",
                    MonitorFormApplication(
                        answer_text=msd_settings.no_form_app_def_answer,
            ...
            if intent_result.numero_protocollo:
                if not next(
                    (
                        domanda
                        for domanda in list_forms.listaDomande
                        if intent_result.numero_protocollo[0] in domanda.numeroProtocollo
                    ),
                    None,
                ):
                    if len(list_forms.listaDomande) >= 1:
                        return RagOrchestratorResponse(
                            ...
                            "",
                            MonitorFormApplication(
                                answer_text=msd_settings.no_spec_form_app_show_list,
                    ...
                    user_form_application = next(
                        (
                            domanda
                            for domanda in list_forms.listaDomande
                            if domanda.numeroProtocollo == intent_result.numero_protocollo[0]
                        ),
                        None,
                    )
            ...
            if intent_result.numero_domus:
                if not next(
                    (
                        domanda
                        for domanda in list_forms.listaDomande
                        if intent_result.numero_domus[0] in domanda.numeroDomus
                    ),
                    None,
                ):
                    if len(list_forms.listaDomande) >= 1:
                    ...
                    user_form_application = next(
                        (
                            domanda
                            for domanda in list_forms.listaDomande
                            if domanda.numeroDomus == intent_result.numero_domus[0]
                        ),
                        None,
                    )
            ...
            clog_params = CLogParams(
                cf=request.user_fiscal_code,
                prestazione=tag,
                num_domus=user_form_application.numeroDomus,
                num_prot=user_form_application.numeroProtocollo,
            )
            clog_last_status = CLog(
                ret_code=0, err_desc=None, id_event=clog_settings.msd_dettagliodomande, params=clog_params
            )

            try:
                form_application_details = await domus.a_get_form_application_details(
                    DomusFormApplicationDetailsRequest(
                        user_form_application.numeroDomus, user_form_application.progressivoIstanza, request.token
                    ),
                    session,
                    logger,
                )
            ...
            if form_application_details and (
                form_application_details.errore
                or not string.is_null_or_empty_or_whitespace(form_application_details.messaggioErrore)
            ):
                clog_last_status.ret_code = 200
            ...
            if request.conversation_id and not redis:
                ...
                logger.track_event(
                    event_types.event_track_log_redis_cache,
                    {
                        "Set_Redis_cache": json.dumps(
                            form_application_details.model_dump_json(), ensure_ascii=False
                        ).encode("utf-8")
                    },
                )
                logger.track_event(
    ...
    try:
            ...
            if domus_result.has_answer and domus_result.answer:
                ...
                if wrong_input and (request.text_by_card is None or request.text_by_card.strip() == ""):
                    ...
                    return RagOrchestratorResponse(
                        ...
                        "",
                        MonitorFormApplication(
                            answer_text=domus_result.answer, event_type=EventMonitorFormApplication.show_answer_text
                        ),
                        clog_last_status,
                    )
        ...
        if tag_info.id_monitoring_question == EnumMonitorFormApplication.OnlyMonitoringQuestion.value:
            return RagOrchestratorResponse(
                ...
                "",
                MonitorFormApplication(
                    answer_text=monitor_form_app.default_answer,
                    event_type=EventMonitorFormApplication.show_answer_text,
                ),
                clog_last_status,
            )
