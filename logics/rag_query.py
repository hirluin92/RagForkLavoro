...
async def a_execute_query(
    ...
    if domusData:
        search_result_context.append(
            RagContextContent("", domusData, len(search_result_context) + 1, "", 100, request.tags[0])
        )

    if len(search_result_context) == 0:
        return RagQueryResponse(llm_const.default_answer, [], "", [], False, [], [], [])

    interactions = extract_chat_history(request.interactions)
    response_from_llm = await a_get_response_from_llm(
        request.query, request.lang, search_result_context, prompt_data, logger, interactions, consumer
    )
    ...
    for item in search_result_context:
        ...
        best_document = BestDocument(
            item.chunk_id, "", item.filename, item.tags, "", item.score, item.chunk, item.reference
        )
...
def build_question_context_from_search(search_result: SearchIndexResponse) -> list[RagContextContent]:
    ...
    for value in search_result.value:
        ...
        content.append(
            RagContextContent(value.chunk_id, value.chunk_text, index, value.filename, score, ", ".join(value.tags))
        )
        index += 1
...
async def a_get_response_from_llm(
    question: str,
    lang: str,
    ...
    if llm_model_id == llm_const.mistralai:
        return await mistralai_get_answer_from_context(question, lang, context_to_send, prompt_data, logger)
    else:
        return await openai_get_answer_from_context(
            question, lang, context_to_send, prompt_data, logger, interactions, consumer
        )


def build_response_for_user(
    rag_response: RagResponse, context: list[RagContextContent]
) -> tuple[str, list[str], list[int]]:
    if len(rag_response.references) > 0:
        documents: list[str] = []
        for reference in rag_response.references:
            filename_to_add = next((x for x in context if x.reference == reference), None)
            if filename_to_add:
        ...
        if len(documents) > 0 and len(documents) == len(rag_response.references):
            ...
            raise Exception(errMsg.format(references=len(rag_response.references), documents=len(documents)))
