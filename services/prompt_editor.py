...
async def a_get_response_from_prompt_retrieval_api(promptId: str,
    ...
    async with session.get(endpoint,
        ...
        result_json_string = json.dumps(
            result_json, ensure_ascii=False).encode('utf-8')
        track_event_data = {
...
async def a_get_prompts_data(prompt_editor: list[PromptEditorCredential],
    ...
    for prompt_type in prompt_types:
        ...
        payload.append(PromptEditorRequest(
            prompt_data.id, prompt_data.version, prompt_data.label))

    listPrompt = await a_get_response_from_prompts_api(logger, session, payload)

    enrichment_data = next(
        (p for p in listPrompt if p["label"] == llm_const.enrichment), None)
    completion_data = next(
        (p for p in listPrompt if p["label"] == llm_const.completion), None)
    msd_intent_recognition_data = next(
        (p for p in listPrompt if p["label"] == llm_const.msd_intent_recognition), None)
    msd_completion_data = next(
        (p for p in listPrompt if p["label"] == llm_const.msd_completion), None)
...
async def a_get_response_from_prompts_api(logger: Logger,
    ...
    async with session.post(endpoint,
        ...
        logger.track_event(
            event_types.prompts_api_result_response, track_event_data)
...
async def get_prompt_id_and_version(prompt_editor: list[PromptEditorCredential],
    ...
    prompt_data = next(
        (p for p in prompt_editor if p.type == prompt_type), None)
    if not prompt_data:
        prompt_data = next(
            (p for p in list_prompt_version_info if p.type == prompt_type), None)
        if not prompt_data:
...
async def a_get_prompt_from_resolve_jinja_template_api(logger: Logger,
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(resolve_endpoint,
                                data=json.dumps(
                                    body.to_dict(), ensure_ascii=False).encode('utf-8'),
                                headers=headers) as result:
            result_json = await result.json()
            result_json_string = json.dumps(
                result_json, ensure_ascii=False).encode('utf-8')
            if result.status == 200:
                ...
                logger.track_event(
                    event_types.resolve_template_api_request, track_event_data)
...
async def a_get_form_application_name_by_tag(container_name: str, tag: str, logger: Logger) -> tuple:
    ...
    raise Exception(
        f"No domus form application name or code found for '{tag}'.")
