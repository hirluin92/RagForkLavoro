...
from utils.settings import get_mistralai_settings, get_openai_settings, get_search_settings, get_storage_settings
...
async def test_get_from_index_ko(mocker, monkeypatch):
    ...
    request = {"question": "question", "interactions": ["tag"]}

    mocker.patch("aiohttp.ClientSession.post", side_effect=Exception)
...
async def test_get_from_index_ok(mocker, monkeypatch):
    ...
    mock_response_json_answer = {"key": "key", "text": "text", "highlights": "highlights", "score": 1}
...
async def test_execute_query_empty_search_results_ok(mocker, monkeypatch):
    ...
    mocker.patch("logics.rag_query.openai_generate_embedding_from_text", return_value=mock_embedding)
    ...
    result = await a_execute_query(
        request, mock_prompt_data, mock_logger, mock_session, consumer=LLMConsumer("test_consumer", "1234567890abcdef")
    )
    # Arrange
...
async def test_execute_query_mistralai_ok(mocker, monkeypatch):
    ...
    mock_rag_response.references = [1]
    mocker.patch("logics.rag_query.mistralai_get_answer_from_context", return_value=mock_rag_response)
    mock_session = mocker.Mock()
    set_mock_env(monkeypatch)
    request = RagOrchestratorRequest(
    ...
    # Act
    result = await a_execute_query(request, mock_prompt_data, mock_logger, mock_session, consumer=LLMConsumer("test_consumer", "1234567890abcdef"))
    # Arrange
