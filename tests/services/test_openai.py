...
def mock_azure_chat_openai(mocker):
    ...
    mock_message.content = (
        '{"response": "Paris", "standalone_question": "standalone question", "end_conversation": false }'
    )
...
@pytest.mark.asyncio
async def test_openai_get_answer_from_context(mocker, monkeypatch, mock_chat_prompt_template, mock_azure_chat_openai):
    # Arrange
    ...
    mocker.patch("services.openai.check_prompt_variables", return_value=[0])
    ...
    consumer = LLMConsumer("test_consumer", "1234567890abcdef")
    # Act
    result = await openai_get_answer_from_context(
        question, lang, context, mock_prompt_data, mock_logger, interactions, consumer
    )
...
@pytest.mark.asyncio
async def test_do_query_enrichment(mocker, monkeypatch, mock_chat_prompt_template, mock_azure_chat_openai):
    # Arrange
    set_mock_env(monkeypatch)
    mock_logger = MockLogger()
    mock_model_parameters = OpenAIModelParameters(0.0, 0.8, 2000, None)
    ...
    # Act
    result = await openai_get_enriched_query(
        "testo di prova",
        ["auu"],
        "",
        mock_prompt_data,
...
@pytest.mark.asyncio
async def test_get_intent_from_enriched_query(mocker, monkeypatch, mock_chat_prompt_template, mock_azure_chat_openai):
    # Arrange
    set_mock_env(monkeypatch)
    mock_logger = MockLogger()
    mock_model_parameters = OpenAIModelParameters(0.0, 0.8, 2000, None)
    ...
    mock_parser.return_value = mock_parser
    # Act
    result = await a_get_intent_from_enriched_query(
        question, mock_prompt_data, mock_logger, consumer=LLMConsumer("test_consumer", "1234567890abcdef")
    )
...
@pytest.mark.asyncio
async def test_get_answer_from_domus(mocker, monkeypatch, mock_chat_prompt_template, mock_azure_chat_openai):
    # Arrange
    set_mock_env(monkeypatch)
    mock_logger = MockLogger()
    mock_model_parameters = OpenAIModelParameters(0.0, 0.8, 2000, None)
    ...
    # Act
    result = await a_get_answer_from_domus(
        question,
        practice_detail,
        mock_prompt_data,
        mock_logger,
...
async def test_a_resolve_template(mocker, monkeypatch):
    ...
    mock_resolve_template = TemplateResolveResponse(
        resolved_template="text resolved", parameters=[], validation_messages=[]
    )
    mocker.patch("services.openai.a_get_prompt_from_resolve_jinja_template_api", return_value=mock_resolve_template)
    # Act
    result = await a_resolve_template(mock_logger, mock_prompt_data, mock_template_data)
